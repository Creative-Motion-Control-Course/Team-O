...
VelocityGenerator velocity;

// -- Wave Generator --
// TODO: declare 
WaveGenerator1D wave;
...
void setup() {
  ...
  velocity.begin();
  velocity.speed_units_per_sec = 0.0;
  velocity.output.map(&axidraw_kinematics.input_y);

  // -- Configure the wave generator --
  wave.setNoInput();
  wave.frequency = 10.0;
  wave.amplitude = 0.0;
  wave.output.map(&channel_z.input_target_position);
  wave.begin();
  ...
  circle.radius = 0.0;
  circle.rotational_speed_rev_per_sec = 10.0;
  circle.setNoInput(); // we will use the internal clock as the time variable

  // Map circle outputs to X and Y channels
  circle.output_x.map(&axidraw_kinematics.input_x);
  circle.output_y.map(&axidraw_kinematics.input_y);
  circle.begin();
...
void set_speed_y(float32_t speed){
  velocity.speed_units_per_sec = speed;
}
